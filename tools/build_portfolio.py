...
<nav class="section-nav wrap" aria-label="{t('포트폴리오 목차','Portfolio contents')}"><a href="#book">{t('저서','Book')}</a><a href="#work">{t('게임','Games')}</a><a href="#research">{t('연구 · 오픈소스','Research & code')}</a><a href="#about">{t('경력','Experience')}</a><a href="#technology">{t('기술','Technology')}</a><a href="#music">{t('음악','Music')}</a></nav>
